...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/GOODS_ITEMS_121_1455_interrupt.pth', type=str,
                        help='Trained state_dict file path to open. If "interrupt", this will open the interrupt file.')
    parser.add_argument('--top_k', default=75, type=int,
    ...
    parser.add_argument('--config', default='yolact_resnet50_goods_config',
    ...
    parser.add_argument('--images', default='psedo_label:output_folder', type=str,
                        help='An input folder of images and output folder to save detected images. Should be in the format input->output.')
    parser.add_argument('--video', default=None, type=str,
                        help='A path to a video to evaluate on. Passing in a number will use that index webcam.')
    parser.add_argument('--video_multiframe', default=1, type=int,
                        help='The number of frames to evaluate in parallel to make videos play at higher fps.')
    parser.add_argument('--score_threshold', default=0.65, type=float,
...
#
        ...
        for i in range(num_dets_to_consider):
            ...
            class_label=cfg.dataset.class_names[classes[i]]
            ...
            mask_out_1 = img_gpu_masked.cpu().numpy()
            indices = np.where(mask_out_1[:, :, :] == 1)

            x = indices[0][:]
            y = indices[1][:]
            
            corrdinates_list=merge(y, x)
            new_list=[list(t) for t in set(tuple(element) for element in corrdinates_list)]
            dic_coorinates[class_label]=new_list
            ...
            #print(cnt)
            #dic_coorinates[class_label]=cnt
    ...
    json_file=path2.replace('.jpg',".json")
    json_path=os.path.join('json_files',json_file)
    
    def generate_jsonfile(data_dict,img_heigh=h,img_width=w):
    ...
    generate_jsonfile(dic_coorinates)
